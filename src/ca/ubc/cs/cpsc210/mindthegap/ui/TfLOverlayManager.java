...
public class TfLOverlayManager implements MapEventsReceiver {
    ...
    public void updateMarkerOfNearest(Station nearest) {
        ...
        if (!(nearest==null)){
            for(int x=0;x<stnClusterer.getItems().size();x++){
                Marker m = stnClusterer.getItem(x);
    ...
    public boolean longPressHelper(GeoPoint geoPoint) {
        StationManager.getInstance().clearSelectedStation();
        tubeLineOverlays.clear();
        updateOverlays();
        return false;
    }
    ...
    private void markStations() {
        ...
        for (Station s:stnManager){
            StationMarker sm = new StationMarker(mapView,this);
            sm.setIcon(stnIconDrawable);
            sm.setPosition(new GeoPoint(s.getLocn().getLatitude(),s.getLocn().getLongitude()));
            sm.setTitle(s.getName());
            sm.setInfoWindow(stnInfoWindow);
            sm.setRelatedObject(s);
            stnClusterer.add(sm);
        }
    }
    ...
    private void plotLines() {
        ...
        Set<Line> lines = StationManager.getInstance().getSelected().getLines();
        for (Line l : lines){
            for (Branch b: l.getBranches()){
                Polyline p = new Polyline(mapView.getContext());
                p.setColor(l.getColour());
                p.setWidth(getLineWidth(mapView.getZoomLevel()));
                List<GeoPoint> points = new ArrayList<>();
                for (LatLon ll:b.getPoints()){
                    points.add(new GeoPoint(ll.getLatitude(),ll.getLongitude()));
                }
                p.setPoints(points);
                tubeLineOverlays.add(p);
            }
            updateOverlays();
        }
    }
